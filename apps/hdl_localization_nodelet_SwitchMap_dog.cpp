...
private:
  ...
  void points_callback(const sensor_msgs::PointCloud2ConstPtr& points_msg) {
    ...
    // tf 是静态的，等待时间长一点也无所谓
    try {
      if (tf_buffer.canTransform(odom_child_frame_id, points_frame, stamp, ros::Duration(0.5), &tfError)) {
          geometry_msgs::TransformStamped TF_lidar2odom_child_frame = tf_buffer.lookupTransform(odom_child_frame_id, points_frame, stamp, ros::Duration(0.5));
          Eigen::Matrix4f TF_temp = tf2::transformToEigen(TF_lidar2odom_child_frame).cast<float>().matrix();
		      pcl::transformPointCloud(*pcl_cloud, *cloud, TF_temp);
		      cloud->header.frame_id = odom_child_frame_id;
		      //std::cout <<  TF_temp << std::endl;
      } else {
        dzlog_info("cannot be transformed . odom_child_frame_id is %s . points_frame : %s", odom_child_frame_id.c_str(), points_frame.c_str());
        return;
      }
    ...
    if (cloud->header.frame_id.empty()) {
      dzlog_info("cloud->header.frame_id.is empty, shoule be %s . ", odom_child_frame_id.c_str());
      return;
    }
    ...
    if ( (filtered->points.size() < low_points_thresh)  &&  ("livox" == points_frame) )
    {
