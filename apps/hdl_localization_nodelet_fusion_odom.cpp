...
private:
  ...
  void publish_scan_matching_status(const std_msgs::Header& header, pcl::PointCloud<pcl::PointXYZI>::ConstPtr aligned) {
    ...
    if (status.inlier_fraction > LOC_SUCCESS_THRESHLOD) {
      ...
      Eigen::Isometry3d front_odom_pose = Eigen::Isometry3d::Identity();
      Eigen::Isometry3d now_odom_pose = Eigen::Isometry3d::Identity();

      // TODO prediect odom by r2live odometry 只用 1 秒以内的数据
      // ROS_WARN("total odom size is : %d ", odom_data.size());
      if (odom_data.size() < 2) {
        // ROS_WARN(" odom_data.size() < 2, so return :");
        return;
      }

      std::lock_guard<std::mutex> lock(odom_data_mutex);
      for (auto _iter = odom_data.begin(); _iter != odom_data.end(); _iter++) {
        // 时间戳大于 1s 就删除
        if (std::fabs((header.stamp - (*_iter)->header.stamp).toSec()) > 1.0) {
          // erase指向删除后的 后 一个元素
          _iter = odom_data.erase(_iter);
          _iter--;
        }
      }
      // ROS_WARN(" less 1s odom size is : %d ", odom_data.size());
      if (odom_data.size() < 2) {
        // ROS_WARN(" odom_data.size() < 2, so return :");
        return;
      } else {
        ROS_WARN(" now stamp: %lf , last r2live odom :%lf  ", header.stamp.toSec(), (*(odom_data.end() - 1))->header.stamp.toSec());
      }

      bool is_front_odom_pose_ok = false;
      bool is_now_odom_pose_ok = false;

      for (auto _iter = odom_data.begin(); _iter != odom_data.end() - 1; _iter++) {
        geometry_msgs::PoseStamped first_pose, second_pose;

        auto const first_st = (*_iter)->header.stamp.toSec();
        auto const second_st = (*(_iter + 1))->header.stamp.toSec();

        // ROS_INFO(" odom time first_st: %f , second_st: %f", first_st, second_st);

        auto last_odom_st = last_odom_pose.header.stamp.toSec();

        // at start  this no odom ,,need relocate maunal
        if (last_odom_st < 1000.0)
          return;

        if (first_st < last_odom_st && last_odom_st < second_st) {
          first_pose.header = (*(_iter))->header;
          first_pose.pose = (*(_iter))->pose.pose;

          second_pose.header = (*(_iter + 1))->header;
          second_pose.pose = (*(_iter + 1))->pose.pose;
          front_odom_pose = get_pose_at_one_stamap(first_pose, second_pose, last_odom_st);
          ROS_ERROR(" can interlp : last_odom_st :%lf ", last_odom_st);
          // ROS_ERROR_STREAM("now_odom_pose: " << front_odom_pose.matrix() << std::endl);
          is_front_odom_pose_ok = true;
        } else {
          // ROS_WARN(" can not interlp : last_odom_st :%lf ", last_odom_st);
        }

        auto now_odom_st = header.stamp.toSec();
        if (first_st < now_odom_st && now_odom_st < second_st) {
          second_pose.header = (*_iter)->header;
          second_pose.pose = (*_iter)->pose.pose;

          first_pose.header = (*(_iter + 1))->header;
          first_pose.pose = (*(_iter + 1))->pose.pose;
          now_odom_pose = get_pose_at_one_stamap(first_pose, second_pose, now_odom_st);
          // ROS_ERROR(" can interlp : now_odom_st :%lf ", now_odom_st);
          is_now_odom_pose_ok = true;

        } else {
          // ROS_WARN(" can not interlp : now_odom_st :%lf ", now_odom_st);
        }
      }
           */
      Eigen::Isometry3d PoseDelta = Eigen::Isometry3d::Identity();

      // if(is_front_odom_pose_ok && is_now_odom_pose_ok)
      // {
      //   PoseDelta = front_odom_pose.inverse() * now_odom_pose;
      // }
      // else
      // {
      ...
      if (tf_buffer.canTransform(odom_child_frame_id,last_odom_pose.header.stamp , odom_child_frame_id, header.stamp , robot_odom_frame_id, ros::Duration(0.1))) {
        ...
        // ROS_ERROR(" interlp okokookokokokokooooooooooo ");
      }
      else
      {
      ...
      // max_correspondence_dist); return ;
    }
  }

  // geometry_msgs::PoseStamped get_pose_at_one_stamap(const geometry_msgs::PoseStamped start_pt, const geometry_msgs::PoseStamped end_pt, const double now_t)
  Eigen::Isometry3d get_pose_at_one_stamap(const geometry_msgs::PoseStamped start_pt, const geometry_msgs::PoseStamped end_pt, const double now_t) {
    double st = start_pt.header.stamp.toSec();
    double et = end_pt.header.stamp.toSec();
    double dt = et - st;
    double dt_from_st = now_t - st;

    // cout <<  "  dt : " << dt   << " dt_from_st " << dt_from_st << "  lamda :" << dt_from_st / dt << endl;

    // 对平移插值
    Eigen::Vector3d start_pos = Eigen::Vector3d(start_pt.pose.position.x, start_pt.pose.position.y, start_pt.pose.position.z);
    Eigen::Vector3d end_pos = Eigen::Vector3d(end_pt.pose.position.x, end_pt.pose.position.y, end_pt.pose.position.z);

    Eigen::Vector3d now_pcd_tans = start_pos + (end_pos - start_pos) * (dt_from_st / dt);
    ...
    tf::quaternionMsgToTF(start_pt.pose.orientation, start_ori);
    start_ori.normalize();
    ...
    tf::quaternionMsgToTF(end_pt.pose.orientation, end_ori);
    end_ori.normalize();

    tf::Quaternion now_rot = start_ori.slerp(end_ori, (dt_from_st / dt));
    now_rot.normalize();
    // tf::Matrix3x3(now_rot).getRPY(roll, pitch, yaw); // 进行转换

    geometry_msgs::Quaternion qua_msg;
    quaternionTFToMsg(now_rot, qua_msg);

    geometry_msgs::PoseStamped now_pt;
    now_pt.header.stamp = ros::Time().fromSec(now_t);
    now_pt.pose.position.x = now_pcd_tans[0];
    now_pt.pose.position.y = now_pcd_tans[1];
    now_pt.pose.position.z = now_pcd_tans[2];
    now_pt.pose.orientation = qua_msg;

    Eigen::Isometry3d pose_eigen;
    tf2::fromMsg(now_pt.pose, pose_eigen);
